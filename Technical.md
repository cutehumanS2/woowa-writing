...
크루루 서비스의 이메일 발송 기능에 비동기 처리를 적용하며 마주한 문제들을 소개합니다.  
이메일 발송 기능을 구현하는 과정 자체는 다루지 않습니다.


## 초기 코드의 문제점

우리가 원하는 기능은 이메일 발송에 성공했을 때만 발송 내역이 저장되는 것이었습니다.

아래와 같이 EmailFacade에 두 로직을 한 메서드로 묶고, @Transactional을 적용하였습니다.   
이렇게 하면 이메일 발송 실패 시 이메일 발송 내역 저장 로직이 롤백됩니다.
```java
...
```
그러나 이 코드에는 몇 가지 문제점이 있습니다.

**첫 번째, 외부 API 통신이 DB 트랜잭션 범위 안에 속해 있다.**

일반적으로 이메일 전송같이 우리가 제어할 수 없는 외부 API 통신은 DB의 트랜잭션 내에서 제거하는 게 좋습니다.
트랜잭션을 처리하기 위해서는 DB 커넥션이 필요한데, 이 DB 커넥션은 외부 API의 적절한 응답이 올 때까지 기다려야 합니다.
이 대기 시간이 길어지면 병목 현상이 발생할 가능성이 높기 때문에 외부 API 통신 코드를 트랜잭션 내에서 제거하는 게 바람직합니다.

보통 이에 대한 개선책으로 [**퍼사드 패턴**](https://ko.wikipedia.org/wiki/%ED%8D%BC%EC%82%AC%EB%93%9C_%ED%8C%A8%ED%84%B4)이 제시됩니다.
크루루 서비스에는 이미 모든 도메인에 전역적으로 이 퍼사드 패턴이 적용되어 있습니다.
이를 도입하게 된 이유에는 상위 계층을 제공함으로써 코드의 복잡도를 낮추기 위함도 있었지만,
추후 외부 API를 활용한 기능을 구현할 때 앞서 말한 문제점을 방지하기 위함도 있었습니다.
그러나 위 코드와 같이 외부 API 통신 코드를 트랜잭션 범위에 포함하게 되면서 결국 퍼사드 패턴을 제대로 활용하지 못했습니다.

**두 번째, 클라이언트의 응답 대기 시간이 길어져 사용자 경험의 저하가 우려된다.**

말 그대로 이메일 발송이 완료될 때까지 기다려야 하므로 클라이언트의 응답 대기 시간이 길어지는 문제가 발생했습니다.
고작 한 명에게 발송하는 데에도 꽤 긴 시간이 걸리는데, 다수의 사용자에게 메일을 발송한다면 클라이언트가 대기해야 하는 시간이 굉장히 길어질 것으로 보였습니다.
이로 인해 사용자 경험이 저하되는 상황을 예상했습니다.

---

## 해결 방안 1) 이벤트 리스너 + @Async로 비동기 적용하기

첫 번째로 고안해 낸 방안은 이렇습니다.

- 첫 번째 문제 → 트랜잭션이 완료된 후 이메일 발송을 별도의 이벤트 리스너에서 처리
- 두 번째 문제 → 이메일 발송 작업에 비동기를 적용

그렇게 구현한 코드의 일부이고, 비동기는 적용하지 않은 상태입니다.
...
`@TransactionalEventListener`를 사용하여 트랜잭션이 성공한 경우에만(= 발송 내역이 성공적으로 저장된 경우에만) 메일을 전송할 수 있게 하였고,
트랜잭션이 실패하는 경우에도 메일이 발송되어 버리는 문제를 방지하였습니다.

그러나 결국 이 방법을 도입하지 않았습니다. 그 이유는 같은 팀원인 명오의 아주 예리한 지적 때문이었습니다.
...
맞습니다. 이 코드의 경우 save()에서 예외가 발생하면 이후 코드는 실행되지 않습니다.   
즉, 이메일이 발송되지 않는다는 것입니다.
똑같은 결과를 도출한다면 더 간단한 방법을 선택하지 않을 이유가 없어서, 비교적 코드의 복잡도를 올리는 이벤트 리스너를 걷어냈습니다.

---

## 해결 방안 2) @Async + CompletableFuture로 비동기 처리 적용하기
...
이메일 발송 로직에 비동기가 잘 적용되고 있음을 확인할 수 있습니다.

하지만 이 방식에도 문제점이 있었습니다. 이메일이 성공적으로 발송되었는지 여부와 관계없이 발송 내역이 저장되어 버리는 것입니다.

우리에겐 이메일 발송 작업, 즉 비동기 작업의 성공 여부를 확인할 수 있는 장치가 필요했습니다.
이를 위해서 EmailService.send() 메서드가 발송 결과로 Email 객체를 넘겨주면,
그 객체를 발송 내역 테이블에 저장해 주는 방법을 떠올렸습니다.

문제는 @Async만으로는 이 방법을 적용할 수 없었습니다. @Async가 적용된 메서드는 void를 리턴해야 한다는 제약때문이었습니다.
즉, 이메일 발송 작업의 결과를 직접적으로 리턴할 수 없다는 것이죠.

### CompletableFuture 이용

그렇게 추가로 이용한 것이 CompletableFuture 클래스입니다.

CompletableFuture는 자바에서 제공하는 비동기 프로그래밍을 위한 클래스로, 비동기 작업의 결과를 유연하게 처리할 수 있다는 특징이 있습니다.

이를 통해 앞서 말한 `EmailService.send()가 발송 결과로 Email 객체를 리턴하고,
이 객체를 발송 내역 테이블에 저장`하는 로직을 구현하였습니다.

- EmailService

    ```java
        @Async
        // 이메일 발송 결과로 CompletableFuture<Email> 객체를 리턴합니다.
...
이메일 발송 시 파일 첨부 기능을 위해 MultipartFile을 활용하였습니다.

비동기를 적용하고 나니 파일이 첨부된 이메일을 발송할 때마다 `java.nio.file.NoSuchFileException`이 발생했습니다.
...
이는 MultipartFile로 전달된 파일이 임시 저장소에 저장되는 특징 때문이었습니다.

크루루의 경우 application.yml에 아래와 같이 설정을 해주었습니다.
...
file-size-threshold를 2KB로 설정했기 때문에, 첨부 파일이 2KB 초과할 경우 임시 저장소에 저장됩니다.
사실상 거의 모든 파일이 메모리가 아닌 임시 저장소에 저장된다는 것입니다.

문제는 임시 저장소에 저장된 임시 파일은 HTTP 요청이 종료되거나 처리 메서드가 종료되면 자동으로 삭제된다는 점입니다.
그래서 비동기적으로 이메일을 발송하는 로직에서 파일을 첨부하려고 할 때,
파일이 이미 삭제되어 더 이상 접근할 수 없게 되고 NoSuchFileException이 발생합니다.

해결 방법은 간단합니다. 임시 파일을 영구 저장해 주면 됩니다.

크루루는 아래와 같이 EmailFacade와 FileUtil에 임시 파일을 저장하는 메서드를 만들어서 비동기 작업이 시작되기 전 파일을 저장해 주었습니다.
그리고 비동기 작업이 완료된 후 파일을 명시적으로 삭제해 주었습니다.
...
앞에서 했던 것처럼 출력문을 일일이 넣어 메서드들의 실행 순서를 확인하는 것은 매우 비효율적입니다.

이메일 발송 작업이 비동기로 잘 동작하는지 효율적으로 확인하기 위해 테스트 코드를 작성하였습니다.
...
emailFacade.send() 메서드의 호출 전후의 시각을 기록하여 두 시각의 차이에 의존하여 비동기 처리가 적절히 이뤄졌는지 판단하는 방식입니다.

하지만 이 방식은 이메일 발송이나 저장이 실제로 이루어졌는지는 확인할 수 없습니다.
...
Mockito.verify() 메서드를 활용하여 이메일 발송과 발송 내역 저장 작업을 각각 검증하는 방식입니다. 이로써 `시도 1`에서 언급했던 문제는 해결하였습니다.

그러나 이 방식은 테스트가 오래 걸린다는 단점이 남아있습니다. TimeUnit.SECONDS.sleep(2)를 사용하여 최소 2초 동안은 반드시 대기해야 합니다.
...
Awaitility를 사용하면 설정한 시간 내에 특정 조건이 만족될 때까지만 대기하도록 구현할 수 있습니다.
위 코드에서 `await().atMost(2, TimeUnit.SECONDS).untilAsserted(() -> { … });` 처럼 작성하면,
2초 내에 javaMailSender.send() 메서드 호출 한 번과 emailService.save() 메서드 호출 한 번이 성공적으로 수행되면 검증이 바로 완료됩니다.
이메일이 발송되고 발송 내역이 저장되는 작업이 1초만에 끝나면, 아직 2초가 되지 않아도 더 대기하지 않고 바로 검증이 완료된다는 것입니다.

결과적으로 이메일 발송 및 발송 내역 저장 작업의 완료 여부를 효율적으로 검사하고, 불필요한 대기를 줄일 수 있게 되었습니다.
